...
下表是第一轮“机制→源码”导航，保留当时的证据标签作为分析轨迹。后续专项复核已经收录在 `07`—`12B`，最终覆盖和采用结论见 `21`，实现反查见 `21A`。因此本表中的 **S1 初步** 不代表整个项目仍停留在初步阶段。
...
`common/on_action/GM_value_on_action.txt` 已出现 `any_living_character` 与 `every_living_character`。这类逻辑若在季度/月度入口高频执行，规模会随存档人物数增长。专项结论已写入 `06`、`11C` 和 `17A`：新 Mod 使用固定区域、相关人物池、缓存和低频调度，不在核心循环进行世界人物扫描。
...
后续专项文档按下列字段复核，最终实现合同由 `18`—`18B` 冻结：
...
| 原核对项 | 完成位置 |
|---|---|
| on_action 挂接、频率与遍历风险 | `06`、`17A` |
| 皇权变量增减与直接门槛 | `08`、`08A` |
| 党派组织、影响力、分裂与权臣 | `09` |
| 军功、卫所、战争政治与军器 | `10`、`10A` |
| 流官、科举、授职、考成与升迁 | `07` |
| GUI/scripted GUI 与本体覆盖差异 | `13A` |
| CK3 1.19.0.6 和“溥天之下”接口 | `13` |
| 人口、财政、土地、贸易与市场 | `11`—`11C` |
| 属国、殖民、土司与八旗 | `12`—`12B` |
| AI 可用性、有限信息与长期平衡 | `17` |
| 全机制最终覆盖/不采用说明 | `21` |

仍标为 `R` 的具体本体语法和运行时行为，需要在 M0/M1 实机探针中验证；这类不确定性已经进入 `19`、`19A` 和 `19B`，不再作为未分类工作悬置。
